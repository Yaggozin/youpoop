...
// 2. Definição do ID do usuário logado
$logged_in_user_id = $_SESSION['user_id'] ?? null;

// 3. Obtenção dos dados do canal (Compatível com o Roteador e com link antigo ?u=5)
if (isset($channel_data) && is_array($channel_data)) {
    $user = $channel_data;
    $channel_user_id = $channel_data['id'];
} else {
    $identifier = $_GET['user'] ?? $_GET['u'] ?? null;
    ...
    try {
        $stmt_user = $pdo->prepare("SELECT * FROM users WHERE id = :id OR username = :id");
        $stmt_user->execute(['id' => $identifier]);
        ...
        die("Erro ao buscar canal: " . $e->getMessage());
    }
}
...
try { 
    ...
    $stmt_user = $pdo->prepare("SELECT id, profile_icon_path, channel_slogan, channel_banner_path, channel_background_path, featured_video_id, customization, username FROM users WHERE id = ?");
    ...
    $has_banner = !empty($channel_info['channel_banner_path']);
    $db_banner_url = $has_banner ? $channel_info['channel_banner_path'] : null;
...
try { 
    ...
    $stmt_user = $pdo->prepare("SELECT id, username, profile_icon_path, channel_slogan, channel_banner_path, channel_background_path, customization FROM users WHERE id = ?");
    $stmt_user->execute([$channel_user_id]);
    $channel_info = $stmt_user->fetch(PDO::FETCH_ASSOC);
    ...
    $custom_banner_url = $channel_info['channel_background_path'] ?? null;
...
if (!empty($channel_info['featured_video_id'])) {
    try {
        // No seu SQL a tabela é 'videos' e a coluna de imagem é 'thumbnail_path'
        $stmt_featured = $pdo->prepare("SELECT id, title, description, thumbnail_path, duration FROM videos WHERE id = ?");
        $stmt_featured->execute([$channel_info['featured_video_id']]);
        $featured_video = $stmt_featured->fetch(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        // Se der erro, remova o comentário abaixo para ver o que é:
        // echo $e->getMessage();
    }
}
    ...
    <style>
        ...
        * {
        ...
        .upper-section-ytg-box {
            height: 80px;
            background: #202020 url(//web.archive.org/web/20120830190721im_/http://s.ytimg.com/yt/img/channels/c3/bg-channel-header-vfl6TeHdT.png) repeat-x;
            padding: 8px 20px;
            color: white;
        ...
        #channel-body {
            background-image: url('https://web.archive.org/web/20101229125123im_/http://i4.ytimg.com/bg/cNbcQD6zHLPSfcxgFRGiEg/112.jpg?app=bg&v=4d09b62f');
            background-position: top center;
            background-repeat: no-repeat;
            background-size: auto;
            flex-shrink: 0;
        ...
        .button-add-thing:hover {
            background: #f7f7f7ff;
            border: 1px solid #2f91e0ff;
            cursor: pointer;
        }
    ...
    <div id="channel-body" style="<?php echo $body_background_style; ?>">
        ...
        <?php if ($has_banner): ?>
            <div id="channel-banner">
                <img src="<?php echo htmlspecialchars($db_banner_url); ?>" alt="banner">
            </div>
        <?php endif; ?>
            ...
            <div class="upper-section-ytg-box">
                ...
                <div class="top-bar-right">
                    ...
                    <button style="width: 100px; margin-left: 20px; border-radius: 2px; border: 0px solid transparent;">
                        <a href="channel_editor.php" >Editar canal</a>
                    </button>
                    <?php endif; // Fim do bloco if ($is_owner) ?>
                </div>
            </div>
                ...
                <div class="main-content-ytg-box">
                        ...
                        <div class="principal">
                            <div class="placeholder-player">
                                <a href="watch.php?v=<?php echo $featured_video['id']; ?>">
                                    <img src="<?php echo htmlspecialchars($featured_video['thumbnail_path']); ?>" 
                                        alt="Thumbnail" 
                                        style="width: 100%; max-height: 360px; object-fit: cover; display: block;">
                                </a>
                            </div>
                            
                            <div class="baixo" style="padding: 10px; background: #f9f9f9; border: 1px solid #ccc; border-top: none; margin-bottom: 20px;">
                                <h2 class="text-pane" style="margin: 0 0 5px 0;">
                                    <a href="watch.php?v=<?php echo $featured_video['id']; ?>" style="color: #0033cc; text-decoration: none; font-size: 18px; font-weight: bold;">
                                        <?php echo htmlspecialchars($featured_video['title']); ?>
                                    </a>
                                </h2>
                                <p class="text-info" style="font-size: 13px; color: #333; line-height: 1.4;">
                                    <?php echo nl2br(htmlspecialchars($featured_video['description'] ?? 'Nenhuma descrição disponível.')); ?>
                                </p>
                            </div>
                        </div>
                    <?php else: ?>
                        <div class="principal" style="padding: 40px; border: 2px dashed #ccc; text-align: center; background: #fdfdfd; margin-bottom: 20px;">
                            <h2 style="color: #999;">Nenhum vídeo em destaque selecionado.</h2>
                            <?php if ($logged_in_user_id == $channel_info['id']): ?>
                                <p><a href="set_featured.php" style="color: #0033cc; font-weight: bold;">Clique aqui para configurar o seu vídeo principal</a></p>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="upload-container">
                        <?php if (!empty($owner_videos)): ?>
                                ...
                                <div class="upload-content">
                                    <div class="upload">
                                        <div class="thumbnail-content">
                                            ...
                                            <div class="video-time"><?php echo htmlspecialchars($video['time'] ?? '0:00'); ?></div>
                                        </div>

                                        <div class="video-details" style="margin-left: 10px; flex: 1;">
                                            <h2 class="text-pane" style="margin-bottom: 2px;">
                                                <a style="color: #468acaff; font-weight: bold; text-decoration: none;" 
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p style="color: #858585; padding: 10px;">Este canal não tem vídeos.</p>
                        <?php endif; ?>
                    </div>
                </div>
