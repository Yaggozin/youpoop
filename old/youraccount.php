...
// 0.5. cores
// ----------------------------------------------------
// No topo do youraccount.php, após carregar $channel_info do BD:

// Define os padrões (fallbacks) complexos
$default_styles = [
    // Se o canal não customizou, aplicamos o gradiente
    'quadradop' => 'background: linear-gradient(to bottom, #E6E6E6, #c5c5c5, #c5c5c5, #E6E6E6); border: 1px solid #cfcfcf;',
    'left-section-ytg-box' => 'background: linear-gradient(to right, #000000, #c5c5c5, #c5c5c5); border: 1px solid #cfcfcf;',
    'upper-section-ytg-box' => 'background: linear-gradient(to bottom, #414141, #272727); border: 1px solid #333;',
    'sections-ytg-box' => 'background: linear-gradient(to bottom, #595959, #4F4F4F, #343434); border: 1px solid #333;',
];

$final_styles = [];

foreach ($default_styles as $key => $default_css) {
    
    // Pega o valor da coluna correspondente (ex: color_quadradop)
    // O operador ?? serve para garantir que, se a coluna não for encontrada em $channel_info (embora deva ser), ele use '#'
    $saved_color = $channel_info["color_{$key}"] ?? ''; 
    
    // Se a cor salva for o padrão inicial (#FFFFFF), ou vazia, aplicamos o CSS COMPLEXO (gradiente).
    if (empty($saved_color) || $saved_color == '#FFFFFF') {
        $final_styles[$key] = $default_css;
    } else {
        // Se o usuário customizou e salvou outra cor (ex: #FF0000), aplicamos o HEX simples.
        $final_styles[$key] = "background-color: {$saved_color} !important;"; 
    }
}
...
<head>
  ...
  <style>
    ...
    .upper-section-ytg-box {
      ...
      width: 900px;
      height: 80px;
      <?php echo $final_styles['upper-section-ytg-box'] ?? ''; ?>
      padding: 8px;
      color: white;
      border-radius: 10px;
    }
    ...
    .sections-ytg-box {
      ...
      left: 500px;
      width: 900px;
      <?php echo $final_styles['sections-ytg-box'] ?? ''; ?>
      padding: 8px;
      height: 15px;
      border-radius: 0px;
    }
    ...
    .left-section-ytg-box {
      ...
      width: 590px;
      height: 980px;
      <?php echo $final_styles['left-section-ytg-box'] ?? ''; ?>
      padding: 8px;
      border-radius: 10px;
    }
    ...
    .quadradop {
      ...
      width: 590px;
      height: 1000px;
      <?php echo $final_styles['quadradop'] ?? ''; ?>
      padding: 8px;
      border-radius: 10px;
    }
    ...
    .channel-name {
      font-family: <?php echo htmlspecialchars($channel_info['channel_font'] ?? 'Arial, sans-serif'); ?> !important;
      max-width: 44ch;
      white-space: nowrap;
      overflow: hidden;
      text-overflow: ellipsis;
